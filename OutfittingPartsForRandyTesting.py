...
for option in sorted(options):
	...
	if len(options[option]["OUTFITTING PARTS"]) + if len(options[option]["CANVAS"]) > 0:
		...
		for item in sorted(rigging, key=lambda k: (opt, k["VENDOR"], k["PART NUMBER"])):
			pass
	
	if len(options[option]["OUTFITTING PARTS"]) > 0:
		for item in sorted(options[option]["OUTFITTING PARTS"], key=lambda k: (opt, k["VENDOR"], k["PART NUMBER"])):
			row += 1
